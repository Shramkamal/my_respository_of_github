    ...
    <body>
        <section class="is-title-bar">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0">
                    <a href="{{ route('studentPayment.create') }}">Add Student Payment</a>
                </div>
            </div>
        </section>
        <div id="app">
                            ...
                            <tbody>
                                @foreach ($studentPayments as $studentPayment)
                                    <tr class="text-[0.6rem]">
                                        ...
                                        <td>{{ $studentPayment->date_of_payment }}</td>
                                        <td>{{ $studentPayment->note }}</td>            
                                        <td class="actions-cell">
                                            <div class="buttons right nowrap">
                                                <a href="{{ route('studentPayment.edit', $studentPayment->id) }}">
                                                ...
                                                <form action="{{ route('studentPayment.destroy', $studentPayment->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
